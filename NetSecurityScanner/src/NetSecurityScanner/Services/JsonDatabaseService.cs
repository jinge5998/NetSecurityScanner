...
namespace NetSecurityScanner.Services
{
    public class JsonDatabaseService
    {
        ...
        /// <summary>
        /// 保存扫描历史记录（用于综合扫描过程中的初始记录）
        /// </summary>
        /// <param name="historyItem">扫描历史记录项</param>
        /// <returns>保存是否成功</returns>
        public async Task<bool> SaveScanHistoryAsync(ScanHistoryItem historyItem)
        {
            try
            {
                Directory.CreateDirectory(_historyDirectory);
                ...
                if (historyItem.PortScanResults.Count > 0 || historyItem.VulnerabilityResults.Count > 0)
                {
                    string resultFilePath = Path.Combine(_historyDirectory, $"{historyItem.ScanId}.json");
                    CompleteScanResult completeResult = new CompleteScanResult
                    {
                        ...
                        PortScanResults = historyItem.PortScanResults,
                        VulnerabilityResults = historyItem.VulnerabilityResults
                    };
                    string resultJson = JsonSerializer.Serialize(completeResult, new JsonSerializerOptions
                    {
                        WriteIndented = true,
                        IncludeFields = true
                    });
            ...
            {
                Console.WriteLine($"保存扫描历史记录失败: {ex.Message}");
                return false;
            }
        }

        /// <summary>
        /// 更新扫描历史记录（用于综合扫描完成后更新结果）
        /// </summary>
        /// <param name="scanId">扫描ID</param>
        /// <param name="updatedItem">更新后的扫描历史记录项</param>
        ...
        public async Task<bool> UpdateScanHistoryAsync(string scanId, ScanHistoryItem updatedItem)
        {
            try
            {
                ...
                {
                    existingItem.OpenPortsCount = updatedItem.OpenPortsCount;
                    existingItem.VulnerabilitiesCount = updatedItem.VulnerabilitiesCount;
                    existingItem.RiskLevel = updatedItem.RiskLevel;
                    existingItem.PortScanResults = updatedItem.PortScanResults;
                    existingItem.VulnerabilityResults = updatedItem.VulnerabilityResults;
                    existingItem.Duration = updatedItem.Duration;
                }
                else
                {
                    historyList.Add(updatedItem);
                }
                ...
                if (updatedItem.PortScanResults.Count > 0 || updatedItem.VulnerabilityResults.Count > 0)
                {
                    string resultFilePath = Path.Combine(_historyDirectory, $"{scanId}.json");
                    CompleteScanResult completeResult = new CompleteScanResult
                    {
                        ScanId = updatedItem.ScanId,
                        TargetIp = updatedItem.TargetIp,
                        ScanType = updatedItem.ScanType,
                        ScanTime = updatedItem.ScanTime,
                        OpenPortsCount = updatedItem.OpenPortsCount,
                        VulnerabilitiesCount = updatedItem.VulnerabilitiesCount,
                        RiskLevel = updatedItem.RiskLevel,
                        PortScanResults = updatedItem.PortScanResults,
                        VulnerabilityResults = updatedItem.VulnerabilityResults
                    };
                    string resultJson = JsonSerializer.Serialize(completeResult, new JsonSerializerOptions
                    {
                        WriteIndented = true,
            ...
            {
                Console.WriteLine($"更新扫描历史记录失败: {ex.Message}");
                return false;
            }
        }
