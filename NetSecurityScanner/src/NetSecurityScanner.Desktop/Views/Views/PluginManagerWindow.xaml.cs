...
namespace NetSecurityScanner.Views
{
    public partial class PluginManagerWindow : Window
    {
        ...
        private Border CreateToolBar()
        {
            ...
            // v7：插件管控中心（仅管理员）
            var govBtn = CreateToolbarButton("⚙ 插件管控器", "#8E44AD", OpenPluginGovernor_Click);
            panel.Children.Add(govBtn);

            var remoteGovBtn = CreateToolbarButton("🌐 远程管控", "#2C3E50", OpenRemoteGovernorConsole_Click);
            panel.Children.Add(remoteGovBtn);
        ...
        private async void OpenPluginGovernor_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (!string.Equals(SessionContext.Instance.Current?.Username, "admin", StringComparison.OrdinalIgnoreCase))
                {
                    MessageBox.Show("仅管理员可访问管控中心。", "权限不足", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }

                // 确保 Governor 已初始化（依赖 PluginManager + Orchestrator）
                if (!PluginGovernor.Instance.IsInitialized)
                {
                    var manager = new Plugins.PluginManager();
                    await manager.LoadAllPluginsAsync();
                    var orchestrator = PluginOrchestrator.Instance;
                    if (!orchestrator.IsInitialized) await orchestrator.InitializeAsync();
                    await PluginGovernor.Instance.InitializeAsync(manager, orchestrator);
                }

                var gov = new PluginGovernorWindow { Owner = this };
                gov.ShowDialog();
            }
            catch (Exception ex)
            {
                MessageBox.Show($"打开管控中心失败: {ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }

        private async void OpenRemoteGovernorConsole_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (!string.Equals(SessionContext.Instance.Current?.Username, "admin", StringComparison.OrdinalIgnoreCase))
                {
                    MessageBox.Show("仅管理员可访问远程管控台。", "权限不足", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                var console = new RemoteGovernorConsoleWindow { Owner = this };
                console.ShowDialog();
            }
            catch (Exception ex)
            {
                MessageBox.Show($"打开远程管控台失败: {ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
